...
func (wac *Conn) Login(qrChan chan<- string) (Session, error) {
	...
	//oldVersion=8691
	login := []interface{}{"admin", "init", []int{0, 3, 3320}, []string{wac.longClientName, wac.shortClientName}, session.ClientId, true}
...
func (wac *Conn) RestoreSession(session Session) (Session, error) {
	...
	//admin init
	init := []interface{}{"admin", "init", []int{0, 3, 3320}, []string{wac.longClientName, wac.shortClientName}, session.ClientId, true}
